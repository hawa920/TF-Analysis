 ...
 * After partioning the file into N equal parts,
...
void pthread_counting(void *pstruct)
{
  ...
  int id = ((dt_pthread *) pstruct) -> id;
  struct timeval sp = ((dt_pthread *) pstruct) -> sp, ep = ((dt_pthread *) pstruct) -> ep;
...
*/
  ...
  printf("%d : %f secs\n", id, ep.tv_sec - sp.tv_sec + (float) (ep.tv_usec - sp.tv_usec) / 1000000);
...
{
  int n_thread;
  char *key_path, *txt_path;
  key_path = strdup(argv[1]);
  txt_path = strdup(argv[2]);
